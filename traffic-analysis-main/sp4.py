...
PATH_CONFIGS = 'configs/speed4.yaml'
cfg = get_config()
...
def main():
    ...
    while True:
        ...
        # Draw line for reference path to estimation speed
        arr_line = cfg.MAIN.LINE
        for i, line in enumerate(arr_line):
            arr_line = np.array(line)
            cv.polylines(frame, [arr_line], True, (0, 255, 0), thickness=1)
            if(i == 2):
                cv.line(frame, line[0], line[1], (0, 255, 0), 2)
                dp = calculate_distance_p2p(line[0], line[1])
                cv.putText(frame, '4.4 m: {:.2f} dp'.format(dp), (line[0][0] + 40, line[0][1] + 10), cv.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,0), 2)
        # Area Get Speed
                ...
                if tracking_list_obj is None:
                    ...
                    for i, track_obj in enumerate(tracking_list_obj):
                        ...
                        if(distance_obj <= cfg.MAIN.LIMIT_DISTANCE):
                            ...
                            # check improve speed
                            for idx, line in enumerate(cfg.MAIN.LINE):
                                if (idx != 2):
                                    sp_poly1 = Polygon(line)
                                    sp_poly2 = Polygon(tracking_list_obj[i][8])
                                    intersect = sp_poly1.intersects(sp_poly2)
                                    if(idx == 0 and intersect):
                                        tracking_list_obj[i][11][0] = 1
                                    if(idx == 1 and intersect):
                                        tracking_list_obj[i][11][1] = 1
                                    
                                    if(tracking_list_obj[i][11][0] == 1 and tracking_list_obj[i][11][1] == 1):
                                        # print('sp {} frame', tracking_list_obj[i][11][2])
                                        cv.putText(frame, 'sp {} frame'.format(tracking_list_obj[i][11][2]), (obj_center_x, obj_center_y), cv.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,0), 2)
                                    elif(tracking_list_obj[i][11][0] == 1):
                                        tracking_list_obj[i][11][2] += 1
                                    elif(tracking_list_obj[i][11][1] == 1):
                                        tracking_list_obj[i][11][2] += 1
        ...
        if(count_time_frame >= time_check_get_speed_flow):
            ...
            data = [dt_string, process_time_string, '{:.2f}'.format(fps), '{:.2f}'.format(speed_average_flow), count_car, count_accident, count_vehicle_all]
